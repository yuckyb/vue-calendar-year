<template>
  <div class="calender-box">
    <div class="month-box" v-for="(item, index) in dateList" :key="index">
      <div
        class="date-year-month"
        style="text-align: center;font-size:35rpx;"
      >{{item.year}}年{{item.month}}月</div>
      <div class="weeks">
        <div
          class="week"
          :class="(index==0 || index==6)?'week-active ':''"
      ...
      <div class="days">
        <div
          class="day"
          :class="item.week?'week-active':''"
          v-for="(item, index) in item.days"
          :key="index"
...
import { getLunarDateFun } from "@/utils/utils.js";
var DATE_LIST = [];
